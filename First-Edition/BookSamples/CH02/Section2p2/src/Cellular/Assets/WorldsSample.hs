...
module Cellular.Assets.WorldsSample where

import Cellular.App.Existential.Rules ( RuleImpl(RI), supportedRules )
import Cellular.App.Existential.Worlds ( WorldInstance(..) )
import Cellular.Domain.Automaton
    ( Automaton, RuleCode, CellWorld(..), loadFromFile )

import Cellular.Assets.Automata.GameOfLife ( GoL )
import Cellular.Assets.Automata.Seeds ( Seeds )
import Cellular.Assets.Automata.Replicator ( Replicator )
