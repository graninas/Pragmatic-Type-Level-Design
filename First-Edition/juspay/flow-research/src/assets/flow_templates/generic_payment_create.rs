...
pub trait GenericPaymentCreateFlowTemplate {
  ...
  fn execute(&mut self,
    ...
    // let order_metadata = payment_data.order_metadata.clone();

    // let customer_id = payment_data.customer_id.clone();
    // let customer_profile = self.get_customer(customer_id.clone())?
