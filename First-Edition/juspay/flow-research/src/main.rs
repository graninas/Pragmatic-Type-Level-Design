...
//   b. Merchant → Orchestrator: Sends the payment request.
// Success scenario:
//   1. Orchestrator: Loads the merchant profile.
//        In: <merchant_api_key>
//        Out: <merchant_profile>
//   2. Orchestrator: Determines the best Payment Gateway based on
//        the merchant’s preferences, customer eligibility, and availability.
//      Conditions:
//        - Chosen Payment Gateway has payment confirm and capture procedures.
//        In: <merchant_profile>
//        Out: <chosen_payment_gateway>
//   3. Orchestrator → <chosen_payment_gateway>: We need to create a payment.
//     3a. Payment Gateway requires payment intent procedure.
//               Orchestrator → <chosen_payment_gateway>: Create payment intent without payment method.
//               Orchestrator → <chosen_payment_gateway>: Attach <payment_method>.
//               Orchestrator → <chosen_payment_gateway>: Authorize payment.
//               <chosen_payment_gateway>:
//                 - Requests payment authorization in the bank. Gets approval and authorization code.
//                 - Returns the payment status (payment is authorized).
//     3b. Payment Gateway doesn't handle payment intent procedure.
//               Orchestrator → <chosen_payment_gateway>: Create payment with <payment_method>.
//               Orchestrator → <chosen_payment_gateway>: Authorize payment.
//               <chosen_payment_gateway>:
//                 - Requests payment authorization in the bank. Gets approval and authorization code.
//                 - Returns the payment status (payment is authorized).
//   4. Confirmation.
//     4a. Confirmation is Automatic. Same session.
//         Orchestrator → <chosen_payment_gateway>: Confirm payment.
//             <chosen_payment_gateway>:
//               - Confirms the payment in the bank.
//               - Returns the payment status (payment is confirmed).
//     4b. Confirmation is Manual. Separate session.
//         Orchestrator: Waits for the confirmation.
//         Merchant → Orchestrator (After a while): Confirm the payment.
//         Orchestrator → <chosen_payment_gateway>: Confirm payment.
//             <chosen_payment_gateway>:
//               - Confirms the payment in the bank.
//               - Returns the payment status (payment is confirmed).
//   5. Capture.
//     5a. Capture is Automatic. Same session.
//         Orchestrator → <chosen_payment_gateway>: Capture payment.
//             <chosen_payment_gateway>:
//               - Captures the payment in the bank.
//               - Returns the payment status (payment is captured).
//     5b. Capture is Manual. Separate session.
//         Orchestrator: Waits for the capture.
//         Merchant → Orchestrator (After a while): Capture the payment.
//         Orchestrator → <chosen_payment_gateway>: Capture payment.
//             <chosen_payment_gateway>:
//               - Captures the payment in the bank.
//               - Returns the payment status (payment is captured).
//   6. Orchestrator → Merchant: Sends the payment status (payment is captured).
// Post-conditions:
//   a. Merchant → Client: Notifies the payment is successful.
